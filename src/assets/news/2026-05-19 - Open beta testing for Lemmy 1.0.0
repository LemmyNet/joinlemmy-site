...
If you run a test instance for Lemmy, we suggest that you upgrade it to `1.0.0-beta.1` by following the [instructions](https://join-lemmy.org/docs/administration/1.0_upgrade.html). You can also try to run the 1.0 database migrations against a local copy of your production database. This will show how long the upgrade process will take, and help to reveal bugs in the migrations.
